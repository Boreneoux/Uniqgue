...
<header class="bg-transparent text-black py-8 w-full">
    ...
    <div class="container flex justify-around items-center z-50 w-full mx-auto fixed bg-white py-6 top-0 md:items-center md:static md:py-0 md:bg-transparent">
        ...
        <div class=""><a href="<?= base_url('/cart'); ?>"><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-cart4" viewBox="0 0 16 16">
        ...
        <?php if (session()->get('isLoggedIn')) : ?>
            ...
            <?php $firstWord = explode(' ', session()->get('fullname')); ?>
            ...
            <nav id="nav-profile" class="absolute hidden py-5 bg-gray-300 shadow-lg rounded-lg max-w-[250px] w-full right-4 top-24 z-50">
                <ul class="block">
                    <li class="group flex text-dark text-lg mx-8 font-bold py-4">Hello, <?= $firstWord[0]; ?> !</li>
                    <li class="group mx-8 flex py-2 text-base text-dark hover:text-white"><a href="<?= base_url('/user/profile'); ?>">My Profile</a></li>
                    <li class="group mx-8 flex py-2 text-base text-dark hover:text-white"><a href="<?= base_url('/user/logout'); ?>">Logout</a></li>
                </ul>
            </nav>
        ...
        <?php else : ?>
            ...
            <nav id="nav-profile" class="absolute hidden py-5 bg-gray-300 shadow-lg rounded-lg max-w-[250px] w-full right-8 top-24">
                <ul class="block">
                    <li class="group mx-8 flex py-2 text-base text-dark hover:text-white"><a href="<?= base_url('/user'); ?>">Login / Register</a></li>
                </ul>
            </nav>
        <?php endif; ?>
